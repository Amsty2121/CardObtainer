...
namespace up_console
{
    public class MyInformation
    {
        ...
        public async Task<AuthenticationStatuses> DisplayMeAndMyManagerRetryingWhenWrongCredentialsAsync(CredentialsRequest request)
        {
            try
            {
                string username = request.Login;
                SecureString password = SecurePassword(request.Password);
